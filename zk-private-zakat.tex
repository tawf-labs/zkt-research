...
\begin{abstract}
Zakat, a mandatory annual obligation in Islam, redistributes wealth from those above the nisab threshold to eight categories of recipients. Several blockchain-based zakat platforms now exist, but all log complete donor-recipient transactions on public ledgers, exposing who gave, how much, and to whom. This transparency contradicts the Islamic principle of \textit{nafs} (dignity), which maqasid al-shariah treats as one of five essential protections. We built a working prototype on Ethereum Sepolia that lets donors submit zakat through UltraHONK zero-knowledge proofs: a Noir circuit (29 ACIR opcodes) verifies nisab and hawl eligibility client-side, a Pedersen-hash nullifier prevents double-claims, and a four-tier privacy model lets donors choose their disclosure level. Across five benchmark runs (Barretenberg v4.2.1, 16-core Linux), proof generation averaged 246.8 ms; an end-to-end \texttt{donateZK()} transaction on Sepolia (tx \texttt{0x6a0b35\ldots b790}, block 10,854,651) consumed 721,345 gas. Cryptographic privacy and institutional accountability prove compatible: amil institutions verify aggregate compliance on-chain while individual transaction amounts and recipient identities remain private. The protocol contributes a domain-specific privacy layer for Islamic social finance, grounded in nafs protection under maqasid al-shariah.
\end{abstract}
...
Prior work on blockchain zakat has focused on operational efficiency. Khan [2] demonstrated decentralized collection and distribution. Nazeri et al. [3] concluded that ``blockchain's immutable ledger provides complete traceability of zakat funds from donor to asnaf'': precisely the property we identify as problematic. Khairi et al. [4] built a collection system with automated disbursement and an immutable audit trail using ERC-20 tokens. In each case, every transaction (donor address, amount, recipient pool) appears permanently on a public ledger. For zakat recipients, many of whom fall below the nisab threshold, this audit trail reveals socioeconomic status indefinitely.

Islamic jurisprudence gives this concern a name. Al-Shatibi's \textit{al-Muwafaqat} classifies \textit{hifz al-nafs} (preservation of life) among the five \textit{daruriyyat} (necessities) [5], and Ibn Ashur's \textit{Maqasid al-Shariah al-Islamiyyah} extends this to include \textit{al-karamah} (dignity), arguing that exposing financial weakness violates the maqsad of protecting the self [6]. Classical sources treat concealed charity (\textit{sadaqah al-sirr}) as superior to public giving precisely because it guards the recipient's honor. A blockchain ledger that permanently records an individual's zakat eligibility status on Etherscan conflicts with this principle.

Zero-knowledge proofs offer a way out of this tension: prove eligibility cryptographically without revealing the underlying data. Hameed et al. [7] and Wen et al. [8] survey ZKP constructions suitable for blockchain deployment. Noir [10] compiles to an intermediate representation (ACIR) that Barretenberg's UltraHONK prover [9] turns into a succinct proof, verifiable on-chain but hiding the witness. Separately, Buterin et al. [15] argue that ZKPs can reconcile privacy with regulatory compliance.

We combine these primitives into a domain-specific protocol for zakat. The Noir circuit (\texttt{main.nr}, our codebase) encodes two Sharia predicates: (1) total wealth below the nisab threshold of 85,000,000 IDR, and (2) assets held for at least one lunar year (hawl). A Pedersen-hash nullifier, computed as $\text{pedersen}(\mathit{secret}, \mathit{recipient\_address}, \mathit{cycle\_id})$, prevents the same recipient from claiming zakat more than once per cycle without exposing their identity. The full pipeline runs client-side: \texttt{nargo compile} $\rightarrow$ \texttt{nargo execute} $\rightarrow$ \texttt{bb prove}, then the proof and nullifier are submitted to \texttt{ZKTCore.donateZK()} on Ethereum Sepolia for settlement. The research questions are whether off-chain Noir proving plus on-chain nullifier tracking can serve institutional zakat accountability, and whether the resulting protocol satisfies nafs protection under maqasid al-shariah.

The contributions are threefold. First, a Noir circuit that encodes nisab and hawl as verifiable predicates: the circuit's assertion \texttt{total\_wealth < nisab\_threshold} (line 10 of \texttt{main.nr}) directly instantiates the Sharia requirement that zakat recipients must fall below the wealth threshold. Second, a Pedersen-hash nullifier mechanism that prevents re-claiming without identity exposure, enforced on-chain by \texttt{NullifierRegistry.spend()} rejecting duplicate nullifiers. Third, a four-tier donor privacy model (Private, Semi-Private, Verified-Private, Public) that lets donors, not the protocol, decide their disclosure level. The Sepolia prototype generates UltraHONK proofs in 246.8 ms on average ($\sigma = 9.3$ ms across five runs) and completes end-to-end donations in 721,345 gas.

\section{Literature Review}

Blockchain-based zakat management has attracted increasing research interest. Khan [2] demonstrated a blockchain-based decentralized zakat collection and distribution platform that eliminates intermediary delays. Khairi et al. [4] developed a zakat collection blockchain system employing ERC-20 tokens to provide an immutable audit trail. Nazeri et al. [3] explored how blockchain features align with zakat management objectives in Malaysia. A recent Emerald study examined the potential and challenges of blockchain technology for zakat institutions \cite{b_zakat_emerald}. However, these systems share a critical privacy limitation. Complete transparency of donor-recipient relationships on public blockchains enables social stigma, targeted harassment, and pattern analysis attacks [2]. Fauzi et al. [19] confirmed through bibliometric analysis an increasing research trend in Muslim-majority countries. Ascarya [20] demonstrated that Islamic social finance instruments (zakat, infaq, sadaqah, and waqf) play a role in economic recovery, and Unal and Aysan [21] proposed blockchain as a means to harmonize differing Sharia-compliance regimes. Muharam and Osman \cite{b_isf_blockchain} explored the nexus of blockchain and Islamic social finance for sustainability. Yet none of these works integrate privacy-preserving transaction mechanisms.

On the zero-knowledge proof front, Hameed et al. [7] survey ZKP constructions, and Wen et al. [8] analyze zk-SNARK variants suitable for blockchain deployment. Williams \cite{b_zkp_cacm} provides a contemporary overview of ZKPs and their expanding role within blockchain technology. Grassi et al. [11] introduce the Poseidon hash function, which reduces constraint count by up to 8$\times$ compared to Pedersen Hash on EVM chains. For this work, we use Noir [10], a Rust-like DSL that compiles to an ACIR intermediate representation, paired with Barretenberg's UltraHONK prover, which generates proofs that are verified on Ethereum Sepolia through a Solidity verifier contract (\texttt{ZKVerifier.sol}). This approach keeps the proving pipeline client-side (private inputs never leave the donor's device) while anchoring proof hashes on-chain for auditability. Buterin et al. [15] propose ZKP-based privacy-preserving and regulation-compliant frameworks, and M\"{u}hle et al. [18] present zk-SNARK-based identity management suitable for KYC integration.
...
Fig.~\ref{fig:conceptual} shows the three-layer protocol architecture: the off-chain proving pipeline where Noir circuits compile to ACIR and Barretenberg generates UltraHONK proofs at 247 ms, the Sepolia settlement layer hosting ZKTCore and ZakatEscrowManager, and the Next.js PWA frontend with wagmi/XellarKit wallet connectivity.
...
The platform uses a hybrid architecture: Noir circuits on the client side with Ethereum Sepolia for public settlement. Donors pay in stablecoins or ETH. Funds flow through off-chain proof generation and on-chain verification via the \texttt{donateZK()} function. Individual donation privacy is maintained through cryptographic proofs, while institutional accountability is achieved through on-chain event emission and nullifier tracking.

A Progressive Web Application handles the donor workflow: wallet connection, privacy tier selection, donation amount entry, proof generation display, and transaction confirmation. Proof generation runs in a Web Worker ensuring that private inputs never leave the donor's device \cite{b_zkp_blockchain}.
...
In Private mode, the \texttt{donor} field is set to \texttt{address(0)} and \texttt{amount} to zero, revealing nothing about who gave or how much. Semi-Private mode carries the donor's address but zeroes the amount, a deliberate tradeoff for institutional accountability. Neither mode exposes the full \texttt{(donor, amount)} tuple. The proof itself is verified via \texttt{ZKVerifier.verify()} (line 24-37 of \texttt{ZKVerifier.sol}), which checks only a \texttt{keccak256(proof)} hash and nullifier freshness; no identity data appears in the call.
...
\textbf{Double-donation prevention.} The on-chain \texttt{NullifierRegistry} (line 10 of \texttt{NullifierRegistry.sol}) maintains a \texttt{mapping(bytes32 => bool)} of spent nullifiers. The \texttt{spend()} function (line 19-23) enforces single-use semantics with an explicit \texttt{require(!spentNullifiers[nullifier])}: the word ``spent'' captures the one-time-use guarantee. The \texttt{ZKTCore.donateZK()} function (line 553-572) atomically chains \texttt{honkVerifier.verify()} $\rightarrow$ \texttt{nullifierRegistry.spend()} $\rightarrow$ \texttt{zakatEscrowManager.donate()}, so a duplicate nullifier causes the entire transaction to revert. This guarantees each eligible recipient can claim zakat at most once per cycle without the amil institution ever learning the recipient's identity.

\section{Discussion}

\subsection{Sharia Compliance and Deployment}

Al-Shatibi's \textit{al-Muwafaqat} grounds maqasid al-shariah in five \textit{daruriyyat}: the preservation of religion (\textit{din}), life (\textit{nafs}), intellect (\textit{aql}), progeny (\textit{nasl}), and wealth (\textit{mal}) [5]. Ibn Ashur's commentary extends \textit{hifz al-nafs} to encompass dignity (\textit{al-karamah}), arguing that ``exposing an individual's financial weakness to the public constitutes harm to their honour'' [6], an extension that directly applies to blockchain zakat systems. When a public ledger permanently records that a specific address received zakat, it marks that person as below the nisab threshold, a socioeconomic signal that persists indefinitely.

Azizov et al. \cite{b_maqasid_fintech} propose a maqasid al-shariah framework for fintech regulation, and Latang et al. \cite{b_fatwa_crypto} examine the Islamic legal status of cryptocurrency assets. Pramuka et al. \cite{b_icf_blockchain} explore blockchain's role in Islamic charitable crowdfunding for trust and transparency. Our work addresses the donor side through ZK proofs and nullifier-based anonymity; recipient-side privacy through encrypted private notes is reserved for future work.

The protocol satisfies two maqasid dimensions. \textit{Hifz al-nafs} (protection of dignity): Private and Semi-Private modes ensure that neither donation amounts nor full donor-recipient linkages appear on-chain. \textit{Hifz al-mal} (protection of wealth): the \texttt{NullifierRegistry} prevents double-claims, and the ZK proof's soundness guarantee prevents fraudulent eligibility claims. \textit{Amanah} (trustworthiness) for amil institutions is satisfied because the \texttt{ZKVerifier} contract (line 16-35 of \texttt{ZKVerifier.sol}) emits a \texttt{ProofVerified} event containing the proof hash and nullifier, auditable for aggregate compliance without exposing individual transaction data.

Deployment requires three concrete steps beyond the current Sepolia prototype: (1) integration with Indonesia's OJK Fintech Sandbox for regulatory approval, (2) training for amil institution staff on wallet management and proof verification workflows, and (3) migration from the mock IDRX stablecoin to a regulated rupiah-backed digital currency.
...
We built and deployed a privacy-preserving zakat protocol on Ethereum Sepolia that uses Noir circuits and UltraHONK proofs to reconcile two goals that existing blockchain zakat systems treat as contradictory: institutional accountability and individual privacy. The circuit (29 ACIR opcodes) verifies nisab and hawl eligibility client-side; the Pedersen-hash nullifier prevents double-claims without deanonymizing recipients; and the four-tier privacy model lets donors choose their disclosure level. Across five benchmark runs, proof generation averaged 246.8 ms (Barretenberg v4.2.1); an end-to-end Sepolia transaction consumed 721,345 gas.

The core finding is that cryptographic privacy and Sharia compliance are not in tension. Ibn Ashur's framing of \textit{nafs} protection as encompassing financial dignity [6] aligns naturally with ZK proofs: the proof says ``this recipient is eligible'' without saying who they are, how much they earn, or what assets they hold. The \texttt{ZKDonationReceived} event gives amil institutions an auditable record of aggregate zakat flow, while the nullifier registry prevents abuse. What the blockchain records and what it reveals are decoupled for the first time in a zakat-specific protocol.
