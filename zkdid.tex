...
Fig.~\ref{fig:conceptual} depicts the three-layer protocol architecture: the off-chain proving pipeline where Noir circuits compile to ACIR and Barretenberg generates UltraHONK proofs at 262 ms, the Sepolia settlement layer hosting ZKTCore and ZakatEscrowManager, and the Next.js PWA frontend with wagmi/XellarKit wallet connectivity. Fig.~\ref{fig:usecase} shows the donation flow from wallet connection through on-chain verification to SBT receipt to the donor. Fig.~\ref{fig:classdiagram} presents the UML class diagram with contract addresses and cardinality.
