...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.85\columnwidth]{research_positioning.png}
...
Fig.~\ref{fig:conceptual} depicts the three-layer protocol architecture: the off-chain proving pipeline where Noir circuits compile to ACIR and Barretenberg generates UltraHONK proofs at 262 ms, the Sepolia settlement layer hosting ZKTCore and ZakatEscrowManager, and the Next.js PWA frontend with wagmi/XellarKit wallet connectivity. Fig.~\ref{fig:usecase} shows the square-block donation flow from wallet connection through on-chain verification to SBT receipt. Fig.~\ref{fig:classdiagram} presents the smart contract UML class diagram with deployed Sepolia addresses. ZKTCore maintains one-to-many relationships with ZKVerifier, NullifierRegistry, ZakatEscrowManager, and DonationReceiptNFT. ZakatEscrowManager holds a many-to-many relationship with MockIDRX for multi-pool token transfers.
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.90\columnwidth]{usecase.png}
\caption{ZK-PZ Donation Flow: from wallet connection to SBT receipt}
\label{fig:usecase}
\end{figure}

\begin{figure}[!ht]
\centering
\includegraphics[width=0.92\columnwidth]{class_diagram.png}
...
The protocol aligns with maqasid al-shariah \cite{b_maqasid_data,b_maqasid_privacy,b_maqasid_blockchain}. \textit{Nafs} (dignity) protection ensures donor/recipient financial privacy. \textit{Mal} (wealth) uses cryptographic integrity. \textit{Amanah} (trustworthiness) is satisfied through on-chain verification \cite{b_zakat_transparency}. Deployment requires Aztec integration, amil training, and OJK Sandbox alignment.
