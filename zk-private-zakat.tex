...
\begin{itemize}
\item \textbf{RQ1:} Can zero-knowledge proofs reconcile on-chain zakat verification with the Islamic requirement of donor and recipient privacy?
\item \textbf{RQ2:} What protocol architecture achieves this reconciliation while preventing double-claims through cryptographic nullifiers?
\item \textbf{RQ3:} How does the resulting protocol satisfy \textit{nafs} protection under maqasid al-shariah?
\end{itemize}

This paper makes the following contributions:
\begin{enumerate}
\item A Noir-based circuit encoding nisab and hawl eligibility verification as verifiable predicates.
\item A Pedersen-hash nullifier mechanism preventing re-claiming without identity exposure.
\item A four-tier donor privacy model (Private, Semi-Private, Verified-Private, Public).
\end{enumerate}
...
\subsection{Blockchain Zakat Systems}
...
This study employs a Design Science Research (DSR) methodology. DSR is appropriate because the primary contribution is an artifact, namely the ZKT protocol, whose design must be iteratively evaluated against Sharia requirements and performance benchmarks. Alternative methodologies such as experimental design were considered but rejected because the research goal is not to test a hypothesis about existing systems but to create and evaluate a new privacy-preserving protocol.

The methodology follows five phases, as mapped in Fig.~\ref{fig:dsr}. Phase 1 identifies the research problem through a literature search spanning blockchain zakat, ZKP privacy frameworks, and Islamic finance instruments across 29 papers published between 2021 and 2025. The search confirmed that no existing blockchain zakat platform implements donor-recipient privacy and that no ZKP framework incorporates Islamic eligibility predicates. Phase 2 designs the ZKT protocol, the Noir circuit architecture encoding nisab and hawl verification, and the four-tier privacy model. Design alternatives, including Circom-based circuits and Aztec's hybrid public-private state model, were evaluated and rejected because Circom prioritizes constraint optimization over code readability for business logic, while the Aztec model introduces additional infrastructure dependencies. Phase 3 implements the system using nargo v1.0.0-beta.21, Barretenberg v4.2.1, and 15 Solidity contracts deployed via a single forge script to Sepolia. The contracts include ZKTCore, ZKVerifier, NullifierRegistry, ZakatEscrowManager, ShariaReviewManager, and supporting components. A Next.js 15 PWA frontend with wagmi and XellarKit provides wallet connectivity, with proof generation isolated inside a Web Worker. Phase 4 demonstrates the prototype on Sepolia testnet. Five benchmark runs were conducted on a 16-core Linux machine, measuring proof generation time (average 247 ms), verification time (12.0 ms), and end-to-end donation gas (721,345 gas at block 10,854,651). Success was defined as: zero donor-recipient pairs exposed in Private mode, 100\% duplicate nullifier rejection, and complete proof verification within a single transaction. Phase 5 evaluates the protocol through three dimensions: Foundry gas reports for computational cost, security properties analysis described in Section IV, and a manual Sharia compliance review conducted with input from Tawf Labs domain experts against the maqasid al-shariah framework.

The three research questions map to this methodology as follows. RQ1 (ZKPs for privacy) is addressed by Phase 2 (designing verifiable Sharia predicates in Noir) and Phase 4 (demonstrating zero-donor-recipient exposure on-chain). RQ2 (protocol architecture for double-claim prevention) is addressed by Phase 3 (implementing the nullifier registry and ZKTCore) and Phase 4 (testing duplicate nullifier rejection). RQ3 (nafs protection under maqasid) is addressed by Phase 5 (Sharia compliance evaluation).

Several validity threats are acknowledged. Internal validity is limited by the use of a single benchmark machine and a single testnet (Sepolia). External validity is constrained by the Indonesian regulatory context and the reliance on Ethereum-specific infrastructure. The Sepolia deployment does not reflect Ethereum mainnet gas costs or network conditions. Construct validity concerns the operationalization of \textit{nafs} protection: the protocol measures on-chain privacy outcomes but does not empirically assess whether these outcomes translate to recipient perceptions of dignity preservation. The Sharia compliance review involved manual assessment by domain experts, which introduces potential researcher bias. The Sepolia deployment (v8) and benchmark data are available at \url{https://github.com/tawf-labs/zkt-hackathon} to support independent replication.
